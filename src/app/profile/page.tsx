...
function ProfileContent() {
  ...
  useEffect(() => {
    ...
    if (savedModel) {
      const validModels = ["gemini-2.5-flash", "gemini-3-flash-preview", "gemini-3.1-pro-preview", "gemini-1.5-flash", "gemini-1.5-pro"];
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <main className="container" style={{ paddingTop: '8.5rem' }}>
        ...
        <section className="glass-card animate-fade-in" style={{ marginBottom: '3rem', padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <UserButton />
            <div>
              <div style={{ fontSize: '0.9rem', color: '#cbd5e1', fontWeight: 'bold' }}>
                {isSignedIn ? (user.fullName || user.emailAddresses[0].emailAddress) : t('profile.no_account')}
              </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', marginBottom: '4rem' }}>
          ...
          {/* AI Settings (Only for Dev Mode) */}
          {hasDevMode && (
            <section className="glass-card animate-fade-in" style={{ border: '1px solid rgba(16, 185, 129, 0.2)' }}>
              <h2 style={{ fontSize: '1.1rem', marginBottom: '2rem', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span>🤖</span> AI解析設定（開発者用）
              </h2>
              
              <div style={{ marginBottom: '2rem' }}>
                <label style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.8rem' }}>Google Gemini API Key</label>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <input
                    type="password"
                    value={apiKey}
                  ...
                  <button 
                    ...
                    disabled={isTesting}
                    style={{ padding: '0.8rem 1rem', background: '#1e293b', border: '1px solid #334155', borderRadius: '8px', color: 'white', cursor: 'pointer', fontSize: '0.8rem' }}
                  >
                    {isTesting ? "..." : "テスト"}
                  </button>
                </div>
              ...
              <div style={{ marginBottom: '2rem' }}>
                <label style={{ display: 'block', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.8rem' }}>解析モデル</label>
                <select
                  value={selectedModel}

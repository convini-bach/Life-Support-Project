...
## 7. ナレッジベース（bach brain）の優先参照
- **コンテキストの同期**: 新しい作業や提案を開始する前には、必ず `bach brain/` ディレクトリ内（`practice/`, `dictionary/`, `開発ログ` など）を検索・参照し、過去の決定事項やルールを読み込んでから思考を開始すること。AIの推測よりも、ユーザーが保管した外部脳の記述を最優先とする。

## 8. AIの回答フォーマット（Obsidian特化）
- **リンクとネットワークの形成**: セッション内で生成する回答や提案は、このプロジェクトが Obsidian にインポート・連携されることを前提としたフォーマットにすること。
- **ブラケットの活用**: コード内のファイル名、重要な機能名、コンテキストの概念などに言及する場合は、`[[` と `]]` で囲み、リンクを貼る形式を標準とする（例: `[[index.html]]` を修正します等）。
- **見出しと構成**: 回答時は `### 🧠 検討プロセス` や `### 📝 提案`、`### 🔗 関連タグ` などを適宜使用し、ログとして後から参照される際に整理されたMarkdown構造になるよう意識すること。
